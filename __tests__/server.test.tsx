...
jest.mock('next/headers', () => ({
  ...
  // Mock getUserReviews before requiring server module
  jest.mock('../src/app/server', () => {
    const originalModule = jest.requireActual('../src/app/server');
    return {
      __esModule: true,
      ...originalModule,
      getUserReviews: jest.fn().mockResolvedValue(['review1', 'review2']),
    };
  });
  ...
  describe('Server Functions', () => {
    ...
    describe('signIn', () => {
      it('should sign in a user and set cookies', async () => {
        ...
        const mockUsersCollection = {
          authWithPassword: jest.fn().mockResolvedValue(mockUser),
        };
        ...
        expect(mockUsersCollection.authWithPassword).toHaveBeenCalledWith(
          'test-user@vanderbilt.edu',
          'Testpass123!'
        );
      ...
      it('should throw an error if email or password is missing', async () => {
        ...
        await expect(signIn(formData)).rejects.toThrow('Invalid email or password');
      });
    ...
    describe('register', () => {
      it('should register a new user and set cookies', async () => {
        ...
        expect(mockCookies.set).toHaveBeenCalledWith('reviews', mockUser.reviews);
        expect(result).toEqual(mockUser);
      });
      ...
      it('should throw an error if email is not a Vanderbilt email', async () => {
        ...
        await expect(register(formData)).rejects.toThrow(
          'Only Vanderbilt email addresses are allowed.'
        );
      });
